...
from utils_seir import joint_pos, post_samples, hdi
...
post_save = pd.DataFrame(
    np.column_stack(
        (
            jnp.squeeze(pos_samples1.theta),
            jnp.squeeze(pos_samples1.p_reported),
            jnp.squeeze(pos_samples1.alpha),
        )
    )
)
...
def pairplot(postdata):
    ...
    g = sns.PairGrid(postdata, diag_sharey=False, corner=True, despine=True)
    ...
    g.figure.savefig("./marposseir1.pdf")
