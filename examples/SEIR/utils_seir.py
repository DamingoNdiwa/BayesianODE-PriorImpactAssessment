...
def post_samples(num_results=3000, num_burnin_steps=1000,
                 key=1, phi=5):
...
def joint_pos1(a=5):
    ...
    def model():
        ...
        inv_alpha = yield tfd.JointDistributionCoroutine.Root(tfd.Sample(
            tfd.Gamma(concentration=tf.cast(a, dtype='float64'),
                      rate=tf.cast(a, dtype='float64')), name='alpha'))
...
def post_samples1(num_results=3000, num_burnin_steps=1000,
                  key=1, a=5):
    """ function to get posterior samples
