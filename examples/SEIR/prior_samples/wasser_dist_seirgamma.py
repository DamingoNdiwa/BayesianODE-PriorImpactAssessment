...
def model():
    ...
    inv_alpha = yield tfd.JointDistributionCoroutine.Root(
        tfd.Sample(
            tfd.Gamma(
                concentration=tf.cast(16, dtype="float64"),
                rate=tf.cast(16, dtype="float64"),
            ),
            name="alpha",
        )
    )
...
d3_prior = dist3_prior.sample(num_samples, seed=key)


# save the prior in the form that can be used for Wasserstein distance
# calculation by Ott
d3 = np.column_stack((d3_prior.theta, d3_prior.p_reported, d3_prior.alpha))
