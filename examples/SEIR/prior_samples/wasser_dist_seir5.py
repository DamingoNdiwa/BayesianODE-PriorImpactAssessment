...
def model():
    ...
    inv_alpha = yield tfd.JointDistributionCoroutine.Root(
        tfd.Sample(tfd.Exponential(rate=tf.cast(5, dtype="float64")), name="alpha")
    )
...
refd = np.column_stack((refd_prior.theta, refd_prior.p_reported, refd_prior.alpha))
