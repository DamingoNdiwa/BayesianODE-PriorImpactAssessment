...
pos_samples0 = post_samples(
    num_results, num_burnin_steps, key, u_init=jnp.log(jnp.array([10, 10.0]))
)

# Second prior
pos_samples1 = post_samples(
    num_results, num_burnin_steps, key, u_init=jnp.log(jnp.array([2.0, 2.0]))
)

# Third prior

pos_samples2 = post_samples(
    num_results,
    num_burnin_steps,
    key,
...
posterior_predictive0 = jax.vmap(
    lambda key, sample: dist.sample(seed=key, value=sample)
)(keys, pos_samples0)
...
axs[0].legend(
    fontsize="small",
    ncol=2,
    bbox_to_anchor=(0.45, -0.1, 0.56, 1.8),
    frameon=True,
    title="Hare",
...
axs[1].legend(
    fontsize="small",
    ncol=2,
    bbox_to_anchor=(0.45, -0.1, 0.55, 1.3),
    frameon=True,
    title="Lynx",
...
axs[0, 0].plot(year, data[:, 0], "ko", mfc="none", ms=2.5, label="observed hare")
axs[0, 0].tick_params(axis="both", labelsize=12)
axs[0, 0].set_ylabel("Pelts (thousands)", fontsize=12)
axs[0, 0].legend(
    fontsize="small",
    ncol=2,
    bbox_to_anchor=(0.45, -0.1, 0.56, 1.48),
    frameon=True,
    title="Hare",
...
axs[1, 0].plot(year, data[:, 0], "ko", mfc="none", ms=2.5, label="observed hare")
...
axs[1, 0].fill_between(
    year, pi0[0][:, 0], pi0[1][:, 0], color="k", alpha=0.3, interpolate=False
)
axs[1, 0].fill_between(
    year, pi0[0][:, 1], pi0[1][:, 1], color="b", alpha=0.3, interpolate=True
)
axs[1, 0].set_ylabel("Pelts (thousands)", fontsize=12)
axs[1, 0].legend(
    fontsize="small",
    ncol=2,
    bbox_to_anchor=(0.45, -0.1, 0.56, 1.1),
    frameon=True,
    title=r"$p_0$",
    title_fontsize="small",
)
axs[1, 0].text(1825, 220, s="\\textbf{c}", fontsize=20)
axs[1, 1].plot(year, data[:, 0], "ko", mfc="none", ms=2.5, label="observed hare")
...
axs[1, 1].fill_between(
    year, pi1[0][:, 0], pi1[1][:, 0], color="k", alpha=0.3, interpolate=False
)
axs[1, 1].fill_between(
    year, pi1[0][:, 1], pi1[1][:, 1], color="b", alpha=0.3, interpolate=True
)
...
axs[2, 0].plot(year, data[:, 0], "ko", mfc="none", ms=2.5, label="observed hare")
...
axs[2, 0].fill_between(
    year, pi2[0][:, 0], pi2[1][:, 0], color="k", alpha=0.3, interpolate=False
)
axs[2, 0].fill_between(
    year, pi2[0][:, 1], pi2[1][:, 1], color="b", alpha=0.3, interpolate=True
)
...
axs[2, 0].legend(
    fontsize="small",
    ncol=2,
    bbox_to_anchor=(0.40, -0.1, 0.56, 1.14),
    title=r"$p_2$",
    title_fontsize="small",
    frameon=True,
)
axs[2, 0].set_xlabel("Year", fontsize=12)
axs[2, 1].plot(year, data[:, 0], "ko", mfc="none", ms=2.5)
...
axs[2, 1].fill_between(
    year, pi3[0][:, 0], pi3[1][:, 0], color="k", alpha=0.3, interpolate=False
)
axs[2, 1].fill_between(
    year, pi3[0][:, 1], pi3[1][:, 1], color="b", alpha=0.3, interpolate=True
)
axs[2, 1].text(1832, 208, s="\\textbf{f}", fontsize=20)
axs[2, 1].legend(
    fontsize="small",
    ncol=1,
    bbox_to_anchor=(0.8, -0.1, 0.2, 1.13),
    title=r"$p_3$",
    title_fontsize="small",
    frameon=True,
)
