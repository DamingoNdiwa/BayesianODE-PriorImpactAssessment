...
pos_samples0 = post_samples(
    num_results, num_burnin_steps, key=1, u_init=jnp.log(jnp.array([2, 2]))
)

post_save = pd.DataFrame(
    np.column_stack(
        (
            jnp.squeeze(pos_samples0.theta),
            jnp.squeeze(pos_samples0.z_init),
            jnp.squeeze(pos_samples0.sigma),
        )
    )
)
