...
def solve(theta, z_init):
    ...
    # print(theta.shape)
    results = odeint(dz_dt, z_init, t_obs, theta, rtol=1e-6, atol=1e-5, mxstep=1000)
    return results


# The five priors used
# First prior
...
def model():
    theta = yield tfd.JointDistributionCoroutine.Root(
        tfd.Sample(
            tfd.TruncatedNormal(
                loc=tf.cast([1.0, 0.05, 1.0, 0.05], dtype="float64"),
                scale=tf.cast([0.5, 0.05, 0.5, 0.05], dtype="float64"),
                low=tf.cast(0.0, dtype="float64"),
                high=tf.cast(10, dtype="float64"),
            ),
            name="theta",
        )
    )
    z_init = yield tfd.JointDistributionCoroutine.Root(
        tfd.Sample(
            tfd.LogNormal(
                loc=tf.cast(jnp.log(jnp.array([10, 10])), dtype="float64"),
                scale=tf.cast(1.0, dtype="float64"),
            ),
            name="z_init",
        )
    )
    sigma = yield tfd.JointDistributionCoroutine.Root(
        tfd.Sample(
            tfd.LogNormal(
                loc=tf.cast(-1.0, dtype="float64"), scale=tf.cast(1, dtype="float64")
            ),
            sample_shape=(2),
            name="sigma",
        )
    ...
    yield tfd.Independent(
        tfd.LogNormal(loc=jnp.log(solve(theta, z_init)), scale=sigma),
...
def model():
    ...
    z_init = yield tfd.JointDistributionCoroutine.Root(
        tfd.Sample(
            tfd.LogNormal(
                loc=tf.cast(jnp.log(jnp.array([2.0, 2.0])), dtype="float64"),
                scale=tf.cast(1.0, dtype="float64"),
            ),
            name="z_init",
        )
...
def model():
    ...
    z_init = yield tfd.JointDistributionCoroutine.Root(
        tfd.Sample(
            tfd.LogNormal(
                loc=tf.cast(jnp.log(jnp.array([15, 6.0])), dtype="float64"),
                scale=tf.cast(1.0, dtype="float64"),
            ),
            name="z_init",
        )
    ...
    sigma = yield tfd.JointDistributionCoroutine.Root(
        tfd.Sample(
            tfd.LogNormal(
                loc=tf.cast(2.0, dtype="float64"), scale=tf.cast(0.2, dtype="float64")
            ),
            sample_shape=(2),
            name="sigma",
        )
...
def model():
    ...
    sigma = yield tfd.JointDistributionCoroutine.Root(
        tfd.Sample(
            tfd.LogNormal(
                loc=tf.cast(1.0, dtype="float64"), scale=tf.cast(0.1, dtype="float64")
            ),
            sample_shape=(2),
            name="sigma",
        )
...
def model():
    ...
    sigma = yield tfd.JointDistributionCoroutine.Root(
        tfd.Sample(
            tfd.LogNormal(
                loc=tf.cast(1.0, dtype="float64"), scale=tf.cast(0.2, dtype="float64")
            ),
            sample_shape=(2),
            name="sigma",
        )
...
def compute_optimal_transport(a1, a2):
    ...
    print("Entropy regularized OT cost:", jnp.sqrt(ot_solution.reg_ot_cost))

    return print(
        "OT cost without entropy:",
        jnp.sqrt(jnp.sum(ot_solution.matrix * ot_solution.geom.cost_matrix)),
...
# Prior scaled WIM
WsIM01 = (
    compute_optimal_transport(prefd, pd1)[1] / compute_optimal_transport(refd, d1)[1]
)
WsIM02 = (
    compute_optimal_transport(prefd, pd2)[1] / compute_optimal_transport(refd, d2)[1]
)
WsIM03 = (
    compute_optimal_transport(prefd, pd3)[1] / compute_optimal_transport(refd, d3)[1]
)
WsIM04 = (
    compute_optimal_transport(prefd, pd4)[1] / compute_optimal_transport(refd, d4)[1]
)

print("The prior scaled WIM: \n")
print(
