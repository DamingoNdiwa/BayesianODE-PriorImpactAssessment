...
def joint_pos(u_init=jnp.log(jnp.array([10, 10.0])), sigma1=jnp.array([-1, 1])):
    ...
    def model():
        ...
        z_init = yield tfd.JointDistributionCoroutine.Root(
            tfd.Sample(
                tfd.LogNormal(
                    loc=tf.cast(u_init, dtype="float64"),
        ...
        sigma = yield tfd.JointDistributionCoroutine.Root(
            tfd.Sample(
                tfd.LogNormal(
                    loc=tf.cast(sigma1[0], dtype="float64"),
...
def post_samples(
    ...
    key=1,
    u_init=jnp.log(jnp.array([10, 10.0])),
    sigma1=jnp.array([-1, 1]),
):
